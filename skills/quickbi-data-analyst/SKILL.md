...
description: >-
  Quick BI AIPro 数据问答、HTML 报告、报告文档与仪表板生成通道，将自然语言数据问题路由到 AIPro，基于已准备的数据资产完成问数（文字与表格结论）、HTML 报告生成（免登预览与下载链接）、报告文档生成（免登预览链接）与仪表板生成（免登预览链接）。

  触发条件：用户提出数据查询、取数、指标计算、排名、趋势、占比、同比或环比等问题，例如"近 30 天销售额是多少""7 月各渠道出货金额排名"；或要求生成 HTML 报告/页面、报告文档、仪表板、看板、数据报表，例如"生成一份销售分析 HTML 报告""帮我生成一份销售分析报告文档""用销售数据生成看板""把刚才的分析做成仪表板"；或者本对话已调用过本 Skill，用户继续追问数据、调整看板，补充或调整时间范围、分析维度及统计口径。

  不触发条件：同步数据资产、上传文件、创建数据集或数据源；执行数据建模或配置操作，例如修改数据集字段、配置权限；生成代码、文件导出等其他产物；以及与数据分析无关的闲聊、代码编写或文档写作任务。对于前三类请求，应明确告知用户当前 Skill 不支持，并引导其前往 Quick BI 完成相关操作；其他无关请求应交由合适的能力处理。
version: 0.6.0
---

# Quick BI AIPro 数据问答、HTML 报告、报告文档与仪表板

## 概述

问数 + HTML 报告 + 报告文档 + 仪表板入口：把用户数据问题发给 Quick BI 智能问数开放接口。问数轮次取回文字/表格形式的分析结论并原样展示；用户要求 HTML 报告/页面时，服务端用 `qbi-grounded-report` 产出经确定性门禁与一次快速质检的可信单文件 HTML 报告，脚本解析终态 `files` 并输出免登预览/下载链接；用户要求报告文档时，服务端用 `qbi-doc-report` 产出报告文档产物，脚本解析 `<artifact-report>` 标签、签发免登票据并输出预览链接；仪表板轮次（`--dashboard`）由服务端生成仪表板产物，脚本解析 `<artifact-dashboard>` 标签、签发免登票据并输出预览链接。脚本自动附带边界约束，把用户问题原文传给 `--message` 即可，不要自行补充约束或改写。零第三方依赖（Python 3.8+ 标准库）。
...
- 生成报告文档（"帮我生成一份销售分析报告文档""把分析结论整理成文档"）
- 生成/修改仪表板、看板、数据报表（"用销售数据生成看板""把刚才的分析做成仪表板""看板加个趋势图"）
- 对上一轮结果的追问与补充（缩小范围、补时间/渠道/口径），带 `--session-id` 继续（见工作流）
- 会话粘性：本对话用过本 Skill 后，问数与仪表板类需求默认继续走本 Skill；不自行回答数据问题、不改用其他工具，除非用户明确要求

## 能力边界（不触发）

以下三类不支持，**不要**发给服务端、也不要用问数结果拼"近似产物"交差，按话术原样回复：
...
问数途中提出时：先给完已取到的数据结论，再说明边界。判法：诉求是"拿到数据结论"→ 问数轮次；是"HTML 报告/页面"或"报告文档"→ 问数轮次直接提交（服务端自动产出对应产物）；是"可视化看板/报表"→ 仪表板轮次（`--dashboard`）；是"改动数据资产本身"或"其他产物形式"→ 不走。与数据无关（闲聊/写代码/写文档）、用户明确要求换工具的，不走且不用上面的话术。
...
用户要**生成或修改仪表板/看板/报表**时加 `--dashboard`（约束服务端用 `qbi-dashboard-builder` 产出仪表板产物）；**HTML 报告/页面与报告文档属于问数轮次，不加 `--dashboard`**（服务端分别用 `qbi-grounded-report` 产出单文件 HTML 报告、`qbi-doc-report` 产出报告文档产物）；纯问数/分析轮次同样**不要**加：
...
服务端产出报告文档产物时，脚本已自动：① 从 text/reply 中**过滤掉 artifact-report 标签**；② 调用换票接口签发免登票据；③ 按 `settings.yaml` 的 `display_type` **预渲染好可直接粘贴的展示片段** `report.render`：

```json
"report": { "name": "2025年销售分析报告",
...
1. 先照常展示（已过滤标签的）`text` / `reply`；报告文档轮次 reply 常为纯标签，过滤后为空时**不输出空内容**，直接用第 2 步的 render 收尾
2. **紧接着原样输出 `report.render`**（可点击链接形态），**不要用代码块包裹**、不要改写链接
3. 禁止将 artifact-report 标签原文、产物 id/draft-id 等内部字段展示给用户；可告知报告名称（`report.name`）
4. **票据有效期与次数限制**：与仪表板共用换票设置（默认 99999 次 / 99 年）。用户反馈"报告打不开了/过期了"时，用同一 `--session-id` 重新提问让服务端重新输出报告即可重新签票
5. **换票失败降级**：若 report 中无 `url`/`render` 而有 `ticketError`，reply 照常展示（为空时跳过），再用一句话告知用户"报告已生成，但访问链接获取失败"；报障附 ticketError 中的 trace_id，**不要**自行拼接任何 URL 充当访问链接输出
...
1. **能力边界**：命中「能力边界」三类不发本 Skill，不点名本环境可能不存在的具体 skill，不用其他工具变通实现
...
9. **仪表板轮次必须加 `--dashboard`**（含用户点名要饼图/折线图等图表的场景）；HTML 报告/页面、报告文档与纯问数/分析轮次不要加，避免被路由成产物形式
10. **产物渲染直接用脚本预渲染片段**：看板用 `dashboard.render`、HTML 报告用 `html[].render`、报告文档用 `report.render`（脚本已拼好最终形态），原样输出、不进代码块、不自行拼装；换票失败或无文件时不自行拼 URL
