...
description: >-
  Quick BI AIPro 数据问答与仪表板生成通道，将自然语言数据问题路由到 AIPro，基于已准备的数据资产完成问数（文字与表格结论）与仪表板生成，并输出仪表板免登预览链接。

  触发条件：用户提出数据查询、取数、指标计算、排名、趋势、占比、同比或环比等问题，例如"近 30 天销售额是多少""7 月各渠道出货金额排名"；或要求生成/修改仪表板、看板、数据报表，例如"用销售数据生成看板""把刚才的分析做成仪表板"；或者本对话已调用过本 Skill，用户继续追问数据、调整看板，补充或调整时间范围、分析维度及统计口径。

  不触发条件：同步数据资产、上传文件、创建数据集或数据源；执行数据建模或配置操作，例如修改数据集字段、配置权限；生成 HTML 页面、分析报告等非仪表板产物；以及与数据分析无关的闲聊、代码编写或文档写作任务。对于前三类请求，应明确告知用户当前 Skill 不支持，并引导其前往 Quick BI 完成相关操作；其他无关请求应交由合适的能力处理。
version: 0.3.0
---

# Quick BI AIPro 数据问答与仪表板

## 概述

问数 + 仪表板入口：把用户数据问题发给 Quick BI 智能问数开放接口。问数轮次取回文字/表格形式的分析结论并原样展示；仪表板轮次（`--dashboard`）由服务端生成仪表板产物，脚本解析 `<artifact-dashboard>` 标签、签发免登票据并输出预览链接。脚本自动附带边界约束，把用户问题原文传给 `--message` 即可，不要自行补充约束或改写。零第三方依赖（Python 3.8+ 标准库）。
...
3. HTML 页面、分析报告等非仪表板产物

> 当前技能暂不支持该能力。这类需求可以到 Quick BI 上实现。

问数途中提出时：先给完已取到的数据结论，再说明边界。判法：诉求是"拿到数据结论"→ 问数轮次；是"可视化看板/报表"→ 仪表板轮次（`--dashboard`）；是"改动数据资产本身"或"非仪表板产物"→ 不走。与数据无关（闲聊/写代码/写文档）、用户明确要求换工具的，不走且不用上面的话术。
...
用户要**生成或修改仪表板/看板/报表**时加 `--dashboard`（约束服务端用 `qbi-dashboard-builder` 产出仪表板产物；纯问数/分析轮次**不要**加）：
...
9. **仪表板轮次必须加 `--dashboard`**（含用户点名要饼图/折线图等图表的场景）；纯问数/分析轮次不要加，避免被路由成产物形式
10. **看板渲染直接用 `dashboard.render`**：脚本已按 `display_type` 拼好片段（iframe 模式自带可点击链接兜底），原样输出、不进代码块、不自行拼装；换票失败时不自行拼 URL
