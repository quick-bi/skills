...
1. 用 `quickbi-mcp:recall_assets` 发现候选数据集，只取 `asset_type` 为 `dataset` 的在线数据集，其 `origin_asset_id`（即 cubeId）用作 `data_sources[].id`；再用 `quickbi-mcp:get_cube_meta` 按 cubeId 取权威字段清单（`name` / `role` / `granularity`），字段实名与层级粒度以此为准
